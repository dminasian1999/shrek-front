...
const AddProduct: React.FC = () => {
    ...
    const handleChange = (field: keyof Omit<ProductRow, 'imageFile' | 'imageUrl'>, idx: number) =>
        (e: ChangeEvent<HTMLInputElement>) => {
            ...
            setRows(prev => prev.map((r, i) => {
                ...
                updated.profit = updated.buy > 0 ? ((updated.sell - updated.buy) / updated.buy) * 100 : 0;
                return updated;
            }));
        };
    ...
    const handleImageChange = (idx: number) => (e: ChangeEvent<HTMLInputElement>) => {
        ...
        setRows(prev => prev.map((r, i) => (i === idx ? { ...r, imageFile: file, imageUrl: url } : r)));
    };
        ...
        <div className="container app-body">
            <div className="card shadow-lg border-0 rounded-4">
                <div className="card-header bg-primary text-white text-center py-3 rounded-top-4">
                    <h4 className="m-0">🛒 Add New Products</h4>
                </div>
                <div className="card-body">
                        ...
                        <table className="table table-hover align-middle">
                                ...
                                rows.map((row, idx) => (
                                    <tr key={idx}>
                                        <td className="text-center">
                                            {row.imageUrl && (
                                                <img src={row.imageUrl} alt="preview" style={{ width: '40px', height: '40px', objectFit: 'cover' }} className="mb-1 rounded" />
                                            )}
                                        ...
                                        <td>
                                            <div className="d-flex gap-2 align-items-center">
                                                <input type="text" className="form-control bg-transparent" value={row.amount.toFixed(2)} readOnly />
                                                <button className="btn btn-outline-danger" onClick={() => deleteRow(idx)}>
                            ...
                            <tr>
                                <td colSpan={8} className="text-center">
                                    <button className="btn btn-outline-success mt-2" onClick={addRow}>
                            ...
                            {rows.length > 0 && (
                                <tr className="table-light">
                                    ...
                                    <td>
                                        ...
                                        <small className="text-muted d-block">Total Sell</small>
                                        <strong className="d-block text-primary">Income</strong>
                                    </td>
                                    <td>
                                        <small className="d-block">${totalCost.toFixed(2)}</small>
                                        <small className="d-block">${totalSells.toFixed(2)}</small>
                                        <strong className="d-block text-primary">${totalIncome.toFixed(2)}</strong>
                                    </td>
                                </tr>
                            )}
                            </tbody>
                    ...
                    <div className="d-md-none">
                                ...
                                <div key={idx} className="card mb-3 shadow-sm">
                                    <div className="card-body">
                                        <div className="text-center mb-3">
                                            {row.imageUrl && (
                                                <img src={row.imageUrl} alt="preview" className="rounded mb-2" style={{ width: '80px', height: '80px', objectFit: 'cover' }} />
                                            )}
                                            <input type="file" accept="image/*" onChange={handleImageChange(idx)} className="form-control form-control-sm" />
                                        </div>

                                        <div className="mb-2">
                                            <label className="form-label">Name</label>
                                            <input type="text" className="form-control" value={row.name} onChange={handleChange('name', idx)} />
                                        </div>
                                        ...
                                        <div className="mb-2">
                                            ...
                                            <input type="number" className="form-control" value={row.quantity} onChange={handleChange('quantity', idx)} />
                                        </div>

                                        <div className="mb-2 row">
                                            <div className="col-6">
                                                <label className="form-label">Buy (₪)</label>
                                                <input type="number" className="form-control" value={row.buy} onChange={handleChange('buy', idx)} />
                                            </div>
                                            <div className="col-6">
                                                <label className="form-label">Sell (₪)</label>
                                                <input type="number" className="form-control" value={row.sell} onChange={handleChange('sell', idx)} />
                                            </div>
                                        </div>
                                        ...
                                        <div className="mb-2 row">
                                            <div className="col-6">
                                                ...
                                                <input type="text" className="form-control bg-transparent" value={row.profit.toFixed(2)} readOnly />
                                            </div>
                                            <div className="col-6">
                        ...
                        <button className="btn btn-outline-success w-100 mb-3" onClick={addRow}>
                            <i className="fa fa-plus-circle me-2" /> Add New Row
                        </button>
                        ...
                        {rows.length > 0 && (
                            <div className="bg-light p-3 rounded text-end">
                                ...
                                <div>Total Sell: <strong>{`₪${totalSells.toFixed(2)}`}</strong></div>
                                <div>Income: <strong className="text-primary">{`₪${totalIncome.toFixed(2)}`}</strong></div>
                            </div>
                        )}
                    </div>
                    ...
                    <div className="d-grid d-md-flex justify-content-md-end mt-4 gap-2">
                        ...
                        <button className="btn btn-primary" onClick={handleSave} disabled={saving}>
                            {saving ? (
                                ...
                                <><i className="fa fa-save me-2" />Add and Save</>
                            )}
                        </button>
                    </div>
                </div>
